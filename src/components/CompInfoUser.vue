  ...
  <div class="CompSlotDashboard">
    ...
    <div class="container">
      ...
      <div class="row">
        <div class="col-md-2"></div>
        <div class="col-md-8">
          <button type="button" style="width:100%" v-on:click="updateUserInfo(input)" class="btn btn-success">Đồng ý</button>
        </div>
        <div class="col-md-2"></div>
      </div>
    </div>
...
export default {
  ...
  mounted(){
        ...
        this.input.birth_place= this.$store.getters['user/getUser'].birth_place,
        this.input.department= this.$store.getters['user/getUser'].department,
        this.input.phone_number= this.$store.getters['user/getUser'].phone_number,
